...
## 화면 구성 (파란 포스터 스타일 + 반원 계기판 + 지도)
 
레퍼런스(굵은 타이포·원색 배경의 그래픽 스타일)를 참고해 전체를 다시 짰습니다.
 
- 배경: 파란색(#2b46ff) 프레임 안에 흰 시트가 놓인 포스터 레이아웃. 상단에 굵은 제목.
- 중앙 큰 패널 "지금까지의 기록": 값·단위·출처·시각 + 날짜별 기록 표 (카드1·4·5)
  - 반원형 계기판: 좋음(초록)·보통(노랑)·나쁨(주황)·매우나쁨(빨강) 구간 위에서 바늘이 현재 값을 가리킴
  - 정사각형 지도 박스: 대구를 단순화한 실루엣 일러스트(실제 지도 아님) 위에 회색 반투명 레이어가 PM2.5 값에 비례해 짙어짐/옅어짐
- 오른쪽 좁은 패널 "신호가 끊기면?": 5가지 실패를 비유로 설명 + 원래 error_code 병기, 버튼을 세로로 좁게 배치 (카드3)
## 로컬에서 보기
