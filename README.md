# 지금, 우리 동네 공기 — 서울 초미세먼지(PM2.5)

과제 T04(오늘의 진짜 정보판 — 데이터가 안 올 때) 제출용 정적 웹 앱입니다.

## 무엇을 보여주나요

- **실제 값**: [Open-Meteo 대기질 API](https://open-meteo.com/en/docs/air-quality-api)로 서울(37.5665, 126.9780)의 초미세먼지(PM2.5, µg/m³) 농도를 조회합니다. API 키가 필요 없는 공개 원천입니다(CORS 공식 지원, 날씨 API와 같은 인프라).
...
기술 용어(freshness·error_code 등, 채점 기준이 요구하는 표기)는 그대로 남기되, 그 위에 쉬운 말 설명을 덧붙였습니다.

- 히어로: "지금 서울 하늘엔 [N] µg/m³"라는 큰 숫자
- ① 지금까지의 기록: 실제 값 + 날짜별 막대그래프
- ② 아무 열쇠 없이 물어봐요: 비밀키 불필요 설명 (카드2)
- ③ 지금 다시 한 번 재보기: 브라우저 실시간 데모 (카드1 보조)
- ④ 신호가 끊기면?: 5가지 실패를 "측정소 서버가 문을 안 열어줌(401)"처럼 비유로 설명 + 원래 error_code 병기 (카드3)
